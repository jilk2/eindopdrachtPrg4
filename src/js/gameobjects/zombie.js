...
export class Zombie extends Actor {
  ...
  const margin = 150;
  const center = (this.scene?.camera?.pos) ?? (this.scene?.playerOne?.pos) ?? new Vector(viewW / 2, viewH / 2);
  ...
  if (!(x < left || x > right || y < top || y > bottom)) {
    if (Math.random() < 0.5) {
      x = Math.random() < 0.5 ? left - Math.random() * 300 : right + Math.random() * 300;
    } else {
      y = Math.random() < 0.5 ? top - Math.random() * 300 : bottom + Math.random() * 300;
    }
  }
